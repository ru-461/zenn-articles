...
:::message
プレフィックスやコミットメッセージ内容については各々、方針やルールなどがあるかと思いますので、一例としてお読みただければ幸いです。
:::

# コミットメッセージの慣習について

Git を使いコミットする場合、一般的にコミット単位でプレフィックスをつけることが多いようです。
使用するプレフィックスの種類、プレフィックス仕様の有無などは各々のプロジェクトによってルールが定められているようです。
...
プレフィックスを適切につけてコミットログのフォーマットを統一することでコミット、コミットログの検索がしやすくなるそうです。
