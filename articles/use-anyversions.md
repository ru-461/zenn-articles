...
そんな時に活躍するのが、**env系ツール**となります。

Rubyなら[rbenv](https://github.com/rbenv/rbenv)，Node.jsなら[nodenv](https://github.com/nodenv/nodenv)といったように各言語にはOSSとして公開されている有名なenv系ツールが存在します。

タイトルにある「anyenv」は、公式が「All in one for env」と謳っている、オールインワンのバージョンマネージャです。今回はそんなオールインワンなツール「anyenv」を紹介します。
