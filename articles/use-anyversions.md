...
「anyenv」はバージョンマネージャと呼ばれるツールを一元管理するツールになります。「All in one for env」と謳っており、anyenvだけインストールすれば、有名なrbenvやnodenvをまとめて管理できるようになります。今回はそんなオールインワンなツール「anyenv」を紹介します。
