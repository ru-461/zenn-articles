...
最近、GitHub などで Rust 製のコマンドやプロンプトが注目されているのが気になり「ターミナルをもっとおしゃれに使いたい」と思いから、今回は、`Starship` という Rust 製のプロンプトと Rust 製コマンド `exa` を活用してモダンかつ軽量、おしゃれなシェル環境を構築してみました。

作成したターミナル、シェルはこちらです。

![starship x exa で作成したターミナル画像](https://storage.googleapis.com/zenn-user-upload/ci6i4zza6ar0wna6l7zwfjszmsk3)
*プロンプトはStarship、ツリー表示にexaを使用*
...
今回は、**おしゃれなターミナル**というテーマでターミナルの環境構築をしてみました。ターミナル環境が変わると気分転換にもなりコマンドを入力しようとターミナルを開くたびにモチベーションが高まるように感じます。また作業環境にこだわることで生産性が上がるのを体感することが出来ました。
